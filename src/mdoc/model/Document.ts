...
import { DataItem, FullDate, cborDecode, cborEncode } from '../../cbor';
    ...
    const addAttribute = (key: string, value: any) => {
      ...
      if (namespace === DEFAULT_NS) {
        ...
        if (['birth_date', 'issue_date', 'expiry_date'].includes(key) && typeof value === 'string') {
          elementValue = new FullDate(value);
        }

        if (key === 'driving_privileges' && Array.isArray(value)) {
          value.forEach((v, i) => {
            if (typeof v.issue_date === 'string') { elementValue[i].issue_date = new FullDate(v.issue_date); }
            if (typeof v.expiry_date === 'string') { elementValue[i].expiry_date = new FullDate(v.expiry_date); }
          });
        }
      }
